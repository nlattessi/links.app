...
class GetUserLinksTest extends TestCase
{
    ...
    public function test_user_links_show_should_return_a_collection_of_links_associated_to_the_user_optionally_sort_by_name_desc()
    {
        $user = $this->userFactory();
        $token = Auth::tokenById($user->id);

        $this->get(
            "/user/links?token={$token}&sort(name|desc)",
            ['Accept' => 'application/json']
        );
